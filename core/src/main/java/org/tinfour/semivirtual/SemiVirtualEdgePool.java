...
class SemiVirtualEdgePool implements Iterable<IQuadEdge> {
    ...
    /**
     * The constraint maps provide a way of tying a constraint object reference
     * to the edges that are associated with it. This indirect method was
     * introduced in the standard version of Tinfour edges in order to economize
     * on memory use by edges. In the semivirtual variation, we have more
     * flexibility and may be able to eliminate this approach.
     */
    HashMap<Integer, IConstraint> linearConstraintMap = new HashMap<>();

    /**
     * Construct a Edge manager allocating a small number of initial edges.
     *
     */
    SemiVirtualEdgePool() {
        self = this;
...
//  }
    ...
    /**
     * Provides an implementation of iterator which loops through the virtual
     * edge collection, returning the base-edges in order (dual edges are not
     * included).
     *
     ...
     */
    Iterator<IQuadEdge> getIterator(final boolean includeGhostEdges, boolean produceDualEdges) {
        return new Iterator<IQuadEdge>() {
            final boolean skipGhostEdges = !includeGhostEdges;
            ...
            public SemiVirtualEdge next() {
                ...
                if (produceDuals) {
                    // if we are processing Duals, we will only advance position
                    // in memory (iPage and IEdge) after the second side is processed.
                    if (dualSideIsPending) {
                        dualSideIsPending = false;
                        SemiVirtualEdge e = new SemiVirtualEdge(self, page, index ^ 1);
                        hasNext = processHasNext(iPage, iEdge);
                        return e;
                    }

                    // Generate an edge, but do not advance the iPage,iEdge
                    // and leav hasNext as true.
                    dualSideIsPending = true;
                    return new SemiVirtualEdge(self, page, index);
                }

                dualSideIsPending = produceDuals;
