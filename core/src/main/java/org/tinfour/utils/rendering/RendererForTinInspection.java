...
public class RendererForTinInspection {
  ...
  public RenderingSurfaceAid render(int width, int height, int pad) {
    ...
    if (edgeRenderingEnabled) {
      ...
      for (IQuadEdge edge : tin.edges()) {
        ...
        if (edgeLabelEnabled) {
          ...
          r2d = labelLayout.getPixelBounds(frc, 0, 0);
          ...
          double x = xMidpoint - xOffset + xCenter - a * ux;
    ...
    if (vertexRenderingEnabled) {
      ...
      for (Vertex v : tin.vertices()) {
        ...
        e2d.setFrame(xy[2] - 2, xy[3] - 2, 4.0f, 4.0f);
        g2d.draw(e2d);
        g2d.fill(e2d);
   ...
   * @param enabled true if edges are to be drawn; false if edges are not rendered.
   */
  ...
  /**
   * Sets the render operation to label edges.
   * @param enabled true if edges are labeled; false if edge labels are not rendered.
   */
   ...
   * @param enabled true if vertices are to be drawn; false if vertices are not rendered.
   */
  public void setVertexRenderingEnabled(boolean enabled) {
    this.vertexRenderingEnabled = true;
  }
