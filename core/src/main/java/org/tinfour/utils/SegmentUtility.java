 ...
 * represented by {@link IQuadEdge} instances. These operations are
 * particularly relevant in contexts like Delaunay triangulation and mesh refinement.
...
public final class SegmentUtility {
    ...
    /**
     * Computes the diametral circle of a given line segment.
     * The diametral circle is defined as the smallest circle that encloses
     * the segment, where the segment itself forms a diameter of this circle.
     * The center of the diametral circle is the midpoint of the segment,
     * and its radius is half the length of the segment.
     *
     * @param segment the line segment (an {@link IQuadEdge}) for which to compute
     *                the diametral circle; must not be null.
     * @return a {@link Circle} object representing the diametral circle of the segment.
    ...
    public static Circle getDiametralCircle(IQuadEdge segment) {
        ...
        // If length is negative (which implies an issue with the IQuadEdge implementation
    ...
    /**
     * Checks if a given point "encroaches" upon a specified line segment.
     * A point is defined as encroaching a segment if it meets two conditions:
     * <ol>
     *   <li>The point lies strictly inside the diametral circle of the segment.
     *       The "strictness" is determined by the provided {@code tolerance}.</li>
     *   <li>The point is not one of the segment's own endpoints (A or B).
     *       Vertex identity is checked using {@link Vertex#getIndex()}.</li>
     * </ol>
     * This check is fundamental in algorithms like Ruppert's algorithm for Delaunay refinement,
     * where encroached segments must be split.
     *
     * @param point the {@link Vertex} to check for encroachment; must not be null.
     * @param segment the {@link IQuadEdge} segment to check against; must not be null.
     * @param diametralCircleOfSegment the pre-computed diametral circle of the {@code segment},
     *                                 as obtained from {@link #getDiametralCircle(IQuadEdge)};
     *                                 must not be null.
     * @param tolerance a small positive value used in the {@link Circle#isStrictlyInside(double, double, double)}
     *                  check to ensure the point is not too close to the boundary of the
     *                  diametral circle.
     * @return {@code true} if the point encroaches upon the segment; {@code false} otherwise.
     * @throws IllegalArgumentException if {@code point}, {@code segment}, or
     *                                  {@code diametralCircleOfSegment} is null, or if
     *                                  the segment's endpoints are null.
     */
    public static boolean isPointEncroachingSegment(Vertex point,
                                                    IQuadEdge segment,
                                                    Circle diametralCircleOfSegment,
                                                    double tolerance) {
        if (point == null) {
