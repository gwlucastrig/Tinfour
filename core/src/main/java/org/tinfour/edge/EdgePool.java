...
public class EdgePool implements Iterable<IQuadEdge> {
    ...
    /**
     * The constraint map provides a way of tying a constraint object reference
     * to the edges that are associated with it. To conserve memory use, the
     * edge class design only supports the storage of two constraint index
     * values of 12 bits each. But it is possible for a constrained edge to be
     * both the border of a polygon area (requiring two indices) and a linear
     * feature (requiring one index). Thus we require a supplemental way of
     * handing linear constraints.
     */
    ...
    /**
     * Pre-allocates the specified number of edges. For a Delaunay Triangulation
     * with n vertices, there should be 3*n edges.
     *
    ...
    /**
     * Allocates a QuadEdge with null vertices, assigning the responsibility for
     * populating the QuadEdge to the calling application. Because the resulting
     * QuadEdge is part of the QuadEdge collection, it is important that the
     * calling application fully populate the QuadEdge according to its own
    ...
    /**
     * Gets the edge associated with the specified index, if any. If the index
     * value is out of range, or if the associated edge has been deallocated,
    ...
    /**
     * Deallocates all Edges, returning them to the free list. Does not delete
    ...
    public Iterator<IQuadEdge> iterator() {
        return getIterator(true, false);
    }

    /**
     * Constructs an iterator that will optionally skip ghost edges.
     *
     * @param includeGhostEdges indicates that ghost edges are to be included in
     * the iterator production.
     * @param produceDualEdges indicates that both the base and dual edges are
     * to be produced.
     * @return a valid instance of an iterator
     */
    public Iterator<IQuadEdge> getIterator(final boolean includeGhostEdges, boolean produceDualEdges) {
        Iterator<IQuadEdge> ix = new Iterator<IQuadEdge>() {
            final boolean produceDuals = produceDualEdges;
            final boolean skipGhosts = !includeGhostEdges;
            int nextPage;
            int nextEdge;
            boolean dualSideIsPending;
            ...
            /**
             * Overrides the default remove operation with an implementation
             * that throws an UnsupportedOperationException. Tinfour requires a
             * specific set of relationships between edges, and removing an edge
             * from an iterator would damage the overall structure and result in
             * faulty behavior. Therefore, Tinfour iterators do not support
             * remove operations.
             */
            @Override
            public void remove() {
            ...
            public QuadEdge next() {
                if (!hasNext) {
                    // This code will be reached only if the calling application
                    // ignores the return from hasNext.
                    return null;
                }
                QuadEdge currentEdge = pages[nextPage].edges[nextEdge];

                if (produceDuals) {
                    if (dualSideIsPending) {
                        dualSideIsPending = false;
                        hasNext = findNextEdge(nextPage, nextEdge);
                        return currentEdge.getDual();
                    } else {
                        dualSideIsPending = true;
                        return currentEdge;
                    }
                }
     ...
     * insertion point does not necessarily have to lie on the segment. This
     * method splits the segment into two segments so that edge e(a,b) becomes
     * edges p(a,m) and and e(m,b), with forward and reverse links for both
     * segments being adjusted accordingly. The new segment p(a,m) is returned
     * and the input segment e is adjusted with new vertices (m,b).
     * <p>
     ...
     * @return a valid instance of a QuadEdge or QuadEdgePartner (depending on
    ...
    /**
     * Adds the specified constraint to the linear constraint map, thus
     * recording which constraint lies to the left side of the edge.
     *
    ...
    private class Page {
        ...
        /**
         * Sets up the array of free Edges. This method is almost always called
         * when a new page is created. The only time it is not is in the
         ...
         * <p>
         * Note that it is important that deallocation set the QuadEdge back to
         * its initialization states. To conserve processing the allocation
         * routine assumes that any unused QuadEdge in the collection is already
         * in its initialized state and so doesn't do any extra work.
         *
         * @param e a valid QuadEdge
         */
